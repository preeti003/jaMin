...
function FooterMusicPlayer({music}) {
    ...
    const [{id, name, author_name, img, musicName,year}, setCurrTrack] = useState(music);
    ...
    useEffect(()=>{
        ...
        if (isNextClicked){
            var tempName = String(name).replace(" ","-").toLowerCase();
            var tempID;
            fetch(`http://127.0.0.1:5000/login?name=${tempName}&year=${String(year)}`)
            .then((res) => res.json())
            .then((json) => {
                var currRand = Math.floor(Math.random() * json.length);
                var tempName = String(json[currRand].track_name).toLowerCase()
                for(var i in musicDB){
                    var temp = musicDB[i].name.trim().toLowerCase();
                    if(tempName.trim().includes(temp) || temp.includes(tempName.trim())){
                        tempID = musicDB[i].id;
                        break;
                    }
                }
                console.log(tempID);
                let currTrackId = parseInt(tempID);
                if(!tempID){
                    let currTrackId = (id+1) % playlists.length;
                }
                setCookie('id', currTrackId, { path: '/' });
        ...
        if (isPrevClicked){
            let currTrackId = (id) % playlists.length;
            if ((id-1)<0){
    ...
    return (
        <div style={useStyle.component} className={"footer-player"}>
            ...
            <div className="playback-controls">
                ...
                <audio ref={audioElement} src={musicName} preload={"metadata"}/>
