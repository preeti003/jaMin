...
const Search = () => {
    ...
    useEffect(() => {
        setSearchResult(playlists.filter((i) => (
            (i.name && i.name.toLowerCase().startsWith(search)) || (i.author_name && i.author_name.toLowerCase().startsWith(search)) ||
            (i.musicName && i.musicName.toLowerCase().startsWith(search)) || (i.lang && i.lang.toLowerCase().startsWith(search)))
            ));
    }, [search, playlists]);
    return (
        <Container>
